...
path2embeddings = '/home/yash/Desktop/nla_project/pretrained_embeds/glove.6B/glove.6B.' + str(embed_size) + 'd.txt'
...
model = m.vanila_lstm_classifier(embed_size, hidden_dim, num_class)
...
def single_epoch(data, label, train_flag):
	...
	for i in range(len(data)):
		...
		prediction_class = model(embed_ques)
...
for e in range(epochs):
	...
	if val_loss < prev_val_loss:
		...
		torch.save(model.state_dict(), trained_models_path + 'lstm_best_model.pt')